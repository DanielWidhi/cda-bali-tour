...
import { mapTransport } from "@/lib/mappers";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: t("title"), description: t("description"), alternates: { canonical: "/transport" } };
}
...
export default async function TransportPage({
  ...
  const transportOptions = (
    await prisma.transport.findMany({ where: { published: true }, orderBy: { pricePerDay: "asc" } })
  ).map((c) => mapTransport(c, locale as Locale));

  return (
    <div className="mx-auto max-w-7xl px-5 lg:px-8 py-16">
      <div className="max-w-2xl mb-12" data-aos="fade-up">
        <span className="text-xs uppercase tracking-[0.2em] text-[color:var(--color-amber-deep)]">{t("label")}</span>
       ...
       <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 mb-16">
           ...
           <div key={car.slug} data-aos="fade-up" data-aos-delay={(i % 4) * 100}>
             <Card>
              ...
              <CardFooter className="flex-col items-start gap-3 border-t border-black/5 pt-4">
                <div>
                  <p className="font-serif text-xl text-[color:var(--color-amber-deep)]">{formatIDR(car.pricePerDay)}</p>
                  <p className="text-xs text-black/50">/ {car.hoursLabel}</p>
                </div>
                <Button asChild size="sm" className="w-full">
                  <a
                    href={`https://wa.me/${siteConfig.whatsapp}?text=${encodeURIComponent(`Halo, saya ingin sewa mobil ${car.name}`)}`}
                    target="_blank"
        ...
        {transportOptions.length === 0 && <p className="col-span-full text-black/50">{t("noData")}</p>}
      </div>
